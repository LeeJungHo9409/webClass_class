...
public class D09_RegexEmail {
	final static String EMAILS = "조선일보	가정/육아	박은주	zeeny@chosun.com\r\n"
	...
	// emails에 있는 올바른 이메일만 모두 ArrayList에 저장해보세요. (리스트 크기 출력)
	// 어떤 도메인이 몇개나 있는지 세어서 출력해보세요.
...
//				emailMap.putIfAbsent(matcher.group(1),0);
//				System.out.println(emailMap.put(matcher.group(1), emailMap.get(matcher.group(1))+1));
				if((cnt = emailMap.putIfAbsent(matcher.group(1), 1))!=null) {
					emailMap.put(matcher.group(1), cnt+1);
				}
				
				/*
				emailMap.putIfAbsent(matcher.group(1),0);
				sum += emailMap.put(matcher.group(1), emailMap.get(matcher.group(1))+1);*/
		...
		List<Domain> d_cnt = new ArrayList<>();
		
		for(String domain : emailMap.keySet()) {
			d_cnt.add(new Domain(domain, emailMap.get(domain)));
		}
		
		Collections.sort(d_cnt);
		Collections.reverse(d_cnt);
		
		for(Domain domain : d_cnt) {
...
class Domain implements Comparable<Domain>{
	...
	public Domain(String name, Integer cnt) {
		this.name = name;
		this.cnt = cnt;
	}
	...
	public String toString() {
		return String.format("[%s] : %d개 ", name, cnt);
	}
